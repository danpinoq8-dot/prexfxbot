...
const ChartPanel = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (active && data.candles) {
          ...
          if (data.candles.length >= 2) {
            setPrevClose(data.candles[data.candles.length - 2].c);
          }
          setLoading(false);
        }
      } catch (e) {
    ...
    setLoading(true);
    fetchCandles();
    const interval = setInterval(fetchCandles, 10000); // 10s refresh for live feel
      ...
      <svg viewBox={`0 0 ${chartW} ${chartH + 20}`} className="w-full h-full" preserveAspectRatio="none">
        ...
        {Array.from({ length: levels + 1 }, (_, i) => {
          ...
          return (
            <g key={`grid-${i}`}>
              ...
              <text x={2} y={y - 3} fill="hsl(0 0% 40%)" fontSize={8} fontFamily="monospace">
          ...
          return (
            <g key={i}>
              <line
                ...
                stroke={isBull ? "hsl(145 60% 50%)" : "hsl(0 70% 55%)"}
                strokeWidth={1}
              />
              <rect
                x={x} y={bodyTop}
                width={barW} height={bodyH}
                fill={isBull ? "hsl(145 60% 50%)" : "hsl(0 70% 55%)"}
                rx={0.5}
              />
            </g>
          );
        ...
        {lastCandle && (
          <>
            <line
              x1={0} x2={chartW}
              y1={toY(lastCandle.c)} y2={toY(lastCandle.c)}
              stroke={isUp ? "hsl(145 70% 55%)" : "hsl(0 70% 55%)"}
              strokeWidth={0.8}
              strokeDasharray="3 2"
            />
            <rect
              x={chartW - 55} y={toY(lastCandle.c) - 8}
              width={55} height={16} rx={3}
              fill={isUp ? "hsl(145 70% 40%)" : "hsl(0 70% 40%)"}
            />
            <text
              x={chartW - 52} y={toY(lastCandle.c) + 4}
          ...
          {["M15", "H1", "H4", "D"].map((tf) => (
            <button
              ...
              className={`px-2 py-1 rounded text-[9px] font-mono transition-colors ${
                timeframe === tf ? "bg-accent text-accent-foreground" : "bg-secondary/30 text-muted-foreground hover:text-foreground"
              }`}
            >
              {tf}
            </button>
